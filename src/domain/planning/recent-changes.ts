...
export function buildRecentChanges(input: {
  ...
  const chapterChanges = (input.recentChapters ?? []).slice(0, env.PLANNING_RECENT_CHANGES_CHAPTER_LIMIT).map((chapter, index) => ({
  ...
  const riskChanges = (input.riskReminders ?? []).slice(0, env.PLANNING_RECENT_CHANGES_RISK_LIMIT).map((risk, index) => createRecentChangeWithProvenance({
  ...
  const entityChanges = (input.entities ?? [])
    ...
    .slice(0, env.PLANNING_RECENT_CHANGES_ENTITY_LIMIT)
    .map((entity, index) => ({
  ...
  const factChanges = (input.persistedFacts ?? []).slice(0, env.PLANNING_RECENT_CHANGES_FACT_LIMIT).map((fact, index) => createRecentChangeWithProvenance({
  ...
  const eventChanges = (input.persistedEvents ?? []).slice(0, env.PLANNING_RECENT_CHANGES_EVENT_LIMIT).map((event, index) => createRecentChangeWithProvenance({
  ...
  return [...riskChanges, ...factChanges, ...eventChanges, ...entityChanges, ...chapterChanges]
    ...
    .slice(0, env.PLANNING_RECENT_CHANGES_TOTAL_LIMIT);
}
