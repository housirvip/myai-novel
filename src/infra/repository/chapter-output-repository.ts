...
export class ChapterOutputRepository {
  ...
  async createAsync(output: ChapterOutput): Promise<void> {
    await dbRunAsync(
      this.database,
      `
        INSERT INTO chapter_outputs (
  ...
  async getLatestByChapterIdAsync(chapterId: string): Promise<ChapterOutput | null> {
    const row = await dbGetAsync<ChapterOutputRow>(
      this.database,
      `
        SELECT *
        FROM chapter_outputs
